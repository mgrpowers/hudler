...
2. Upload this code as main.py to your ESP32-S3 CIRCUITPY drive
3. Connect ESP32-S3 to Raspberry Pi/computer via USB
4. Run the host application which will send speed values

IMPORTANT: This must be saved as main.py on the CIRCUITPY drive
"""
    ...
    try:
        ...
        if _display_cache['group'] is None:
            ...
            speed_label = label.Label(
                ...
                x=0,  # Position relative to transform group
                y=0,  # Position relative to transform group
                anchor_point=(0.5, 0.5),
                anchored_position=(0, 0),  # Center of transform group
                scale=18  # Large scale to use full width (820 pixels)
            )
            
            # Wrap label in a transform group to flip upside down and mirror for HUD
            # For windshield reflection: text must be upside down and mirrored
            try:
                # Create transform: flip horizontally (scale_x=-1) and vertically (scale_y=-1)
                transform = displayio.Transform(scale_x=-1, scale_y=-1)
                
                # Create a group at the center position with the transform
                transform_group = displayio.Group(x=center_x, y=center_y, scale=(1, 1))
                transform_group.transform = transform
                transform_group.append(speed_label)
                
                group.append(transform_group)
                print(f"  Applied transform group: scale_x=-1, scale_y=-1 (upside down and mirrored)")
            except (AttributeError, ImportError, TypeError) as e:
                print(f"  Transform not available ({e}), trying alternative method...")
                # Fallback: try wrapping in group with negative scale
                try:
                    transform_group = displayio.Group(x=center_x, y=center_y)
                    # Use Group's scale parameter to flip
                    transform_group.scale = (1, -1)  # Vertical flip
                    transform_group.append(speed_label)
                    group.append(transform_group)
                    print(f"  Applied group scale for vertical flip")
                except:
                    # Last resort: just add label directly (hardware flip should handle it)
    ...
    if QUALIA_AVAILABLE and display:
        try:
            ...
            try:
                ...
                transform_group = displayio.Group(x=center_x, y=center_y, scale=(1, 1))
                transform_group.transform = transform
                transform_group.append(idle_label)
                
                group.append(transform_group)
                print(f"  Applied transform group to IDLE: scale_x=-1, scale_y=-1 (upside down and mirrored)")
            except (AttributeError, ImportError, TypeError) as e:
                print(f"  IDLE transform not available ({e}), trying alternative...")
                try:
                    transform_group = displayio.Group(x=center_x, y=center_y)
                    transform_group.scale = (1, -1)  # Vertical flip
                    transform_group.append(idle_label)
                    group.append(transform_group)
                    print(f"  Applied group scale to IDLE for vertical flip")
                except:
